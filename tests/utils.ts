...
export function isFolderAndFiles(folderPath: string, fileList: string[]) {
  // Check if the folder exists
  const folderExists = fs.existsSync(folderPath)
  // Check if each file in the list exists within the folder
  const filesExist = fileList.map((file) => fs.existsSync(path.join(folderPath, file)))

  return { folderExists, filesExist }
}
