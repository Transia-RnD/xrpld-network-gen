...
import { downloadFileAtCommitOrTag, getCommitHashFromServerVersion } from './libs/github'
import {
...
import { type RippledBuild, genConfig } from './rippled_cfg'
import { createDockerfile, downloadBinary } from './utils/deployKit'
import { bcolors, generatePorts, saveLocalConfig } from './utils/misc'
...
async function createStandaloneFolder(
  ...
  fs.writeFileSync(`${basedir}/${protocol}-${name}/genesis.json`, JSON.stringify(genesisJson, null, 4))
  ...
  fs.copyFileSync(`${srcDir}/deploykit/${protocol}.entrypoint`, `${basedir}/${protocol}-${name}/entrypoint`)
...
export async function createStandaloneImage(
  ...
  await createStandaloneFolder(false, name, image, content, networkId, publicKey, importKey, protocol, logLevel)
  services['explorer'] = {
    image: 'transia/explorer:latest',
    container_name: 'explorer',
    environment: ['PORT=4000', `VUE_APP_WSS_ENDPOINT=ws://0.0.0.0:${6006}`],
    ports: ['4000:4000'],
    networks: ['standalone-network'],
  }
  ...
  if (addIpfs) {
    ...
    services['ipfs'] = {
      ...
      volumes: [`${pwdStr}/${protocol}/ipfs_staging:/export`, `${pwdStr}/${protocol}/ipfs_data:/data/ipfs`],
      networks: ['standalone-network'],
    }
  }
...
export async function createStandaloneBinary(
  ...
  const content = await downloadFileAtCommitOrTag(owner, repo, commitHash, 'src/ripple/protocol/impl/Feature.cpp')
  const url = `${buildServer}/${buildVersion}`
  await downloadBinary(url, `${basedir}/${protocol}-${name}/rippled.${name}`)
  const image = 'ubuntu:jammy'
  await createBinaryFolder(true, name, image, content, networkId, publicKey, importKey, protocol, logLevel)
  services['explorer'] = {
    image: 'transia/explorer:latest',
    container_name: 'explorer',
    environment: ['PORT=4000', `VUE_APP_WSS_ENDPOINT=ws://0.0.0.0:${6006}`],
