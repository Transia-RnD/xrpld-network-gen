...
export async function createAnsibleVarsFile(
  dirPath: string,
  ip: string,
  vars: DockerVars
): Promise<DockerVars> {
...
export async function downloadBinary(url: string, savePath: string): Promise<void> {
  ...
  if (fs.existsSync(savePath)) {
    console.log(
      `${bcolors.GREEN}version: ${bcolors.BLUE}${version} ${bcolors.END}already exists...`
    );
...
export async function updateDockerfile(buildVersion: string, savePath: string): Promise<void> {
  ...
  const updatedLines = lines.map(line =>
    pattern.test(line) ? `COPY rippled.${buildVersion} /app/rippled` : line
  );
