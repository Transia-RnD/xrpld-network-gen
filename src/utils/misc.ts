...
export function runStart(cmd: string[], protocol: string, version: string, type: string): void {
  try {
    ...
    if (result.status === 0) {
      console.log(
        `${bcolors.CYAN}${protocol.charAt(0).toUpperCase() + protocol.slice(1)} ${bcolors.GREEN}${version} ${type} running at: ${bcolors.PURPLE}6006 ${bcolors.END}`,
      )
...
export function runLogs(): void {
  try {
    ...
    if (isContainerRunning(containerName)) {
      ...
      console.log()
      console.log(
        `${bcolors.RED}Cannot watch live logs, container not running. Run ${bcolors.PURPLE}./install${bcolors.RED} script${bcolors.END}`,
      )
      console.log()
    }
  } catch (error) {
...
export function generatePorts(index: number, nodeType: string): [number, number, number, number, number] {
  let rpc_public: number
  let rpc_admin: number
  let ws_public: number
  let ws_admin: number
  let peer: number
  if (nodeType === 'validator') {
